...
flow_icis2 <- flow_icis %>%
  ...
  mutate(flow_discharge_ratio = flow_correct/limit_value_nmbr) %>%
...
a <- ggplot(subset(flow_icis2, mean_flow_discharge_ratio != Inf), aes(x = design_flow_round, y = mean_flow_discharge_ratio, size = num_in_bin)) +
...
prep_mean_conc <- function(type, pollutant){
  ...
  data <- data %>%
    ...
    mutate(design_flow_round = plyr::round_any(total_design_flow_nmbr, 0.1, floor),
           design_flow_round_smaller = plyr::round_any(total_design_flow_nmbr, .01, floor),
           design_flow_round_small = plyr::round_any(total_design_flow_nmbr, .001, floor),
           discharge_ratio = mean_conc/limit_value_nmbr, na.rm = TRUE) %>%
    group_by(design_flow_round) %>%
...
prep_mean_quant <- function(type, pollutant){
  ...
  data <- data %>%
    ...
    mutate(design_flow_round = plyr::round_any(total_design_flow_nmbr, 0.1, floor),
           design_flow_round_smaller = plyr::round_any(total_design_flow_nmbr, .01, floor),
           design_flow_round_small = plyr::round_any(total_design_flow_nmbr, .001, floor), 
           discharge_ratio = mean_quant/limit_value_nmbr, na.rm = TRUE) %>%
    filter(!is.na(discharge_ratio)) %>%
    group_by(design_flow_round) %>%
    summarise(mean_discharge_ratio = mean(discharge_ratio, na.rm = TRUE),
              sd_discharge_ratio = sd(discharge_ratio, na.rm = TRUE),
